// Case 1 (LBTC event-only) benchmark runner.
//
// The run cap is the reference Sqd (TS Subsquid) completion time for this
// case, from this case's own README performance table ("Subsquid | 10m"),
// not an arbitrary fixed window — a full run at sqd-go's throughput finishes
// well inside that cap, so this reports genuine time-to-complete for the
// full 20,600,000-22,200,000 range, comparable in shape to the reference
// table entries. Indexers run sequentially (one at a time) to avoid resource
// contention — see the BENCHMARKS registry below. Two sqd-go entries: plain
// "sqd-go" (out-of-the-box --restart) and "sqd-go-max" (--parallel-fetch +
// sqd-go's own Makefile "uniswap-fast" preset) — the gap between them is
// real, not just noise. The other case_1 implementations
// (sentio/envio/ponder/sqd/subgraph) predate this runner and are still
// driven manually per their own READMEs.
...
// Reference Sqd (TS) took 10m to complete this case (see this directory's
// README). Used as a cap, not a target — sqd-go is expected to finish well
// under it.
const REFERENCE_SQD_DURATION_S = 10 * 60;
 ...
 * live and retaining its tail to parse the resource report from. `timeout`
 * (not a manual sleep+kill) is what makes the report reliable — `time`
 * prints once ITS direct child exits, and `timeout` is what cleanly ends it. */
function startTimed(
  cmd: string[],
  cwd: string,
...
async function benchmarkSqdGoImpl(tuned: boolean): Promise<BenchmarkResult> {
  ...
  const args = ["./tmp/sqd-go-bench", "start", SQD_GO_PROJECT_DIR, "--restart"];
  ...
  const result =
    await Bun.$`docker exec ${chContainer} clickhouse-client --password ${rootChPassword} --query "SELECT count(), max(block_number) FROM case_1_lbtc_event_only.lbtc_transfer_events"`
      .quiet()
      .nothrow()
      .text();
  const [eventsStr, blockStr] = result.trim().split("\t");
  ...
  return buildResult(label, totalBlocks, totalEvents, elapsedS, completed, resourceStats);
}
...
async function main() {
  ...
  console.log("=== Case 1: LBTC Event-Only Benchmark ===");
  ...
  const persistResults = () =>
    Bun.write(
      resultsLogPath,
      JSON.stringify({ case: "case_1_lbtc_event_only", updatedAt: new Date().toISOString(), results }, null, 2)
    );

  // Sequential on purpose: each indexer gets ClickHouse/Docker to itself,
  // matching the resource-isolation rationale in cases/*/run.ts.
  for (const name of selected) {
    ...
    console.log(
      `\nSummary — ${result.name} (${status}): ${formatRate(result.blocksPerSec)} blocks/s, ${formatRate(result.eventsPerSec)} events/s${resSuffix} ` +
        `(${result.totalBlocks} blocks, ${result.totalEvents} events)\n`
    );
  }
