#![allow(non_camel_case_types, clippy::enum_variant_names, clippy::too_many_arguments, clippy::upper_case_acronyms, clippy::type_complexity, dead_code)]
// THIS IS A GENERATED FILE. DO NOT MODIFY MANUALLY.
//
// This file was auto generated by rindexer - https://github.com/joshstevens19/rindexer.
// Any manual changes to this file will be overwritten.

use super::rocket_token_reth_abi_gen::{
    RindexerRocketTokenRETHGen::{self, RindexerRocketTokenRETHGenInstance, RindexerRocketTokenRETHGenEvents}
};
...
use super::super::super::super::typings::database::get_or_init_postgres_client;
                    ...
                    pub type ApprovalData = RindexerRocketTokenRETHGen::Approval;
                    ...
                    pub type TransferData = RindexerRocketTokenRETHGen::Transfer;
...
pub fn no_extensions() -> NoExtensions {
    NoExtensions {}
            ...
            type ApprovalEventCallbackType<TExtensions> = Arc<
                dyn for<'a> Fn(&'a Vec<ApprovalResult>, Arc<EventContext<TExtensions>>) -> BoxFuture<'a, EventCallbackResult<()>>
                    + Send
                    + Sync,
                >;

            pub struct ApprovalEvent<TExtensions> where TExtensions: Send + Sync + 'static {
                callback: ApprovalEventCallbackType<TExtensions>,
                context: Arc<EventContext<TExtensions>>,
            }

            impl<TExtensions> ApprovalEvent<TExtensions> where TExtensions: Send + Sync + 'static {
            ...
            #[async_trait]
            impl<TExtensions> EventCallback for ApprovalEvent<TExtensions> where TExtensions: Send + Sync {
                async fn call(&self, events: Vec<EventResult>) -> EventCallbackResult<()> {
                    ...
                    let result: Vec<ApprovalResult> = events.into_iter()
                        .filter_map(|item| {
                            item.decoded_data.downcast::<ApprovalData>()
                                .ok()
            ...
            type TransferEventCallbackType<TExtensions> = Arc<
                dyn for<'a> Fn(&'a Vec<TransferResult>, Arc<EventContext<TExtensions>>) -> BoxFuture<'a, EventCallbackResult<()>>
                    + Send
                    + Sync,
                >;

            pub struct TransferEvent<TExtensions> where TExtensions: Send + Sync + 'static {
                callback: TransferEventCallbackType<TExtensions>,
                context: Arc<EventContext<TExtensions>>,
            }

            impl<TExtensions> TransferEvent<TExtensions> where TExtensions: Send + Sync + 'static {
                pub async fn handler<F, Fut>(closure: F, extensions: TExtensions) -> Self
                where
                    TransferResult: Clone + 'static,
                    F: for<'a> Fn(Vec<TransferResult>, Arc<EventContext<TExtensions>>) -> Fut
            ...
            #[async_trait]
            impl<TExtensions> EventCallback for TransferEvent<TExtensions> where TExtensions: Send + Sync {
                async fn call(&self, events: Vec<EventResult>) -> EventCallbackResult<()> {
                    let events_len = events.len();
                    ...
                    // transfer events with 2 indexed topics cant decode
                    // transfer events with 3 indexed topics
                    let result: Vec<TransferResult> = events.into_iter()
                        .filter_map(|item| {
                            item.decoded_data.downcast::<TransferData>()
                                .ok()
...
pub enum RocketTokenRETHEventType<TExtensions> where TExtensions: 'static + Send + Sync {
    Approval(ApprovalEvent<TExtensions>),
Transfer(TransferEvent<TExtensions>),
}

impl<TExtensions> RocketTokenRETHEventType<TExtensions> where TExtensions: 'static + Send + Sync {
    pub fn topic_id(&self) -> &'static str {
        match self {
            RocketTokenRETHEventType::Approval(_) => "0x8c5be1e5ebec7d5bd14f71427d1e84f3dd0314c0f7b2291e5b200ac8c7c3b925",
RocketTokenRETHEventType::Transfer(_) => "0xddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a4df523b3ef",
        }
    }

    pub fn event_name(&self) -> &'static str {
        match self {
            RocketTokenRETHEventType::Approval(_) => "Approval",
RocketTokenRETHEventType::Transfer(_) => "Transfer",
        }
    }

    pub fn contract_name(&self) -> String {
        "RocketTokenRETH".to_string()
    }
    ...
    fn decoder(&self, network: &str) -> Arc<dyn Fn(Vec<B256>, Bytes) -> Arc<dyn Any + Send + Sync> + Send + Sync> {
        match self {
                    ...
                    RocketTokenRETHEventType::Approval(_) => {
                    ...
                    RocketTokenRETHEventType::Transfer(_) => {
                        Arc::new(move |topics: Vec<B256>, data: Bytes| {
    ...
    pub async fn register(self, manifest_path: &PathBuf, registry: &mut EventCallbackRegistry) {
        ...
        let (callback, reorg_sender): (
            ...
            Option<tokio::sync::broadcast::Sender<rindexer::ReorgEvent>>,
                    ...
                    RocketTokenRETHEventType::Approval(event) => {
                    ...
                    RocketTokenRETHEventType::Transfer(event) => {
                        let reorg_sender = Some(event.context.reorg_tx.clone());
                        let event = Arc::new(event);
                        let callback = Arc::new(move |result| {
                            let event = Arc::clone(&event);
       ...
       registry.register_event(EventCallbackRegistryInformation {
            id: generate_random_id(10),
            indexer_name: "erc20-indexer".to_string(),
