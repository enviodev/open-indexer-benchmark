// What the reliability suite asks of an indexer, and what each answer is worth.
//
 ...
 *
 * `points` is what the claim is worth inside its scenario. They are not all
 * equal on purpose: losing data is not the same finding as taking longer to
 * notice, and a scoring scheme that treated them alike would rank a tool that
 * silently keeps reorged rows above one that pauses for a minute and gets it
 * right.
 */
...
/** Every check's total worth inside its scenario. */
export function scenarioPoints(scenario: Scenario): number {
  return scenario.checks.reduce((sum, check) => sum + check.points, 0);
}
