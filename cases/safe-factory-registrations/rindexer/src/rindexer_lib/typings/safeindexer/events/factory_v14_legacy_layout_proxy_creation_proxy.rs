...
use super::factory_v14_legacy_layout_proxy_creation_proxy_abi_gen::{
    RindexerFactoryV14LegacyLayoutProxyCreationProxyGen::{self, RindexerFactoryV14LegacyLayoutProxyCreationProxyGenInstance, RindexerFactoryV14LegacyLayoutProxyCreationProxyGenEvents}
};
...
pub type ProxyCreationData = RindexerFactoryV14LegacyLayoutProxyCreationProxyGen::ProxyCreation;

#[derive(Debug, Clone)]
pub struct ProxyCreationResult {
    pub event_data: ProxyCreationData,
    pub tx_information: TxInformation,
}

impl HasTxInformation for ProxyCreationResult {
...
pub fn proxycreation_handler<TExtensions, F, Fut>(
    custom_logic: F,
) -> ProxyCreationEventCallbackType<TExtensions>
where
    ProxyCreationResult: Clone + 'static,
    F: for<'a> Fn(Vec<ProxyCreationResult>, Arc<EventContext<TExtensions>>) -> Fut
        + Send
        + Sync
        + 'static
        + Clone,
...
type ProxyCreationEventCallbackType<TExtensions> = Arc<
    dyn for<'a> Fn(
            &'a Vec<ProxyCreationResult>,
...
pub struct ProxyCreationEvent<TExtensions>
where
    TExtensions: Send + Sync + 'static,
{
    callback: ProxyCreationEventCallbackType<TExtensions>,
    context: Arc<EventContext<TExtensions>>,
}

impl<TExtensions> ProxyCreationEvent<TExtensions>
where
    TExtensions: Send + Sync + 'static,
{
        ...
        Fut: Future<Output = EventCallbackResult<()>> + Send + 'static,
    {
        ...
        Self {
            callback: proxycreation_handler(closure),
...
#[async_trait]
impl<TExtensions> EventCallback for ProxyCreationEvent<TExtensions>
where
    TExtensions: Send + Sync,
{
    async fn call(&self, events: Vec<EventResult>) -> EventCallbackResult<()> {
        ...
        let result: Vec<ProxyCreationResult> = events
            .into_iter()
            .filter_map(|item| {
                item.decoded_data
                    .downcast::<ProxyCreationData>()
                    .ok()
                    .map(|arc| ProxyCreationResult {
        ...
        if result.len() == events_len {
            ...
            panic!("ProxyCreationEvent: Unexpected data type - expected: ProxyCreationData")
        }
    }
}

pub enum FactoryV14LegacyLayoutProxyCreationProxyEventType<TExtensions>
where
    TExtensions: 'static + Send + Sync,
{
    ProxyCreation(ProxyCreationEvent<TExtensions>),
}

impl<TExtensions> FactoryV14LegacyLayoutProxyCreationProxyEventType<TExtensions>
where
    TExtensions: 'static + Send + Sync,
{
    pub fn topic_id(&self) -> &'static str {
        match self {
            FactoryV14LegacyLayoutProxyCreationProxyEventType::ProxyCreation(_) => {
    ...
    pub fn event_name(&self) -> &'static str {
        match self {
            FactoryV14LegacyLayoutProxyCreationProxyEventType::ProxyCreation(_) => "ProxyCreation",
        }
    }

    pub fn contract_name(&self) -> String {
        "FactoryV14LegacyLayoutProxyCreationProxy".to_string()
    }
        ...
        match self {
            FactoryV14LegacyLayoutProxyCreationProxyEventType::ProxyCreation(_) => {
                Arc::new(move |topics: Vec<B256>, data: Bytes| {
                    match ProxyCreationData::decode_raw_log(topics, &data[0..]) {
                        Ok(event) => {
                            let result: ProxyCreationData = event;
    ...
    pub async fn register(self, manifest_path: &PathBuf, registry: &mut EventCallbackRegistry) {
        ...
        let (callback, reorg_sender): (
            ...
            FactoryV14LegacyLayoutProxyCreationProxyEventType::ProxyCreation(event) => {
