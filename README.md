...
The run is capped at ten minutes in every scenario. An indexer that has not finished by then is stopped where it got to, and what it did index is verified and rated all the same — so the table reports how much of the data a slow tool holds rather than reporting nothing. Its row is marked as disagreeing with ground truth, and the note below the table leads with the share of the data that is missing and the fact that the range was not finished in time. Storage is left blank for those rows, since a partial database is not comparable with a complete one.
...
**Storage**: on-disk size of the tables the scenario defines, including their indexes, at the data state the verification phase produces — reported only for indexers that completed the range. Each tool's internal bookkeeping is excluded, since it varies with how much a tool caches or retains.
