...
A scenario whose logic cannot be replayed from raw chain data takes its ground truth from a snapshot of its reference implementation instead. Solana's is one: an instruction carries positional accounts and a Borsh payload, and which token a plain `transfer` moved is only knowable by joining the transaction's balances, so writing that a second time in the harness would leave two answers to keep in step rather than one to check against. `expected.json` is then a committed snapshot, and the reference indexer's own row is a regression check — it passes the day the snapshot is taken and fails afterwards if its output changes. Every other indexer is still checked against a reference it had no part in, and the scenario's page says which implementation the snapshot came from.
