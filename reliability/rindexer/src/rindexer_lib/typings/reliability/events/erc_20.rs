...
use super::erc_20_abi_gen::RindexerERC20Gen::{
    self, RindexerERC20GenEvents, RindexerERC20GenInstance,
};
...
pub type TransferData = RindexerERC20Gen::Transfer;
...
pub type MetadataUpdatedData = RindexerERC20Gen::MetadataUpdated;

#[derive(Debug, Clone)]
pub struct MetadataUpdatedResult {
    pub event_data: MetadataUpdatedData,
    pub tx_information: TxInformation,
}

impl HasTxInformation for MetadataUpdatedResult {
    fn tx_information(&self) -> &TxInformation {
        &self.tx_information
    }
}
...
pub fn metadataupdated_handler<TExtensions, F, Fut>(
    custom_logic: F,
) -> MetadataUpdatedEventCallbackType<TExtensions>
where
    MetadataUpdatedResult: Clone + 'static,
    F: for<'a> Fn(Vec<MetadataUpdatedResult>, Arc<EventContext<TExtensions>>) -> Fut
        + Send
        + Sync
        + 'static
        + Clone,
...
type MetadataUpdatedEventCallbackType<TExtensions> = Arc<
    dyn for<'a> Fn(
            &'a Vec<MetadataUpdatedResult>,
            Arc<EventContext<TExtensions>>,
        ) -> BoxFuture<'a, EventCallbackResult<()>>
        + Send
        + Sync,
...
pub struct MetadataUpdatedEvent<TExtensions>
where
    TExtensions: Send + Sync + 'static,
{
    callback: MetadataUpdatedEventCallbackType<TExtensions>,
    context: Arc<EventContext<TExtensions>>,
}

impl<TExtensions> MetadataUpdatedEvent<TExtensions>
where
    TExtensions: Send + Sync + 'static,
{
    pub async fn handler<F, Fut>(closure: F, extensions: TExtensions) -> Self
        ...
        Fut: Future<Output = EventCallbackResult<()>> + Send + 'static,
    {
        let (reorg_tx, _) = tokio::sync::broadcast::channel(16);
        Self {
            callback: metadataupdated_handler(closure),
            context: Arc::new(EventContext {
                database: get_or_init_postgres_client().await,
...
#[async_trait]
impl<TExtensions> EventCallback for MetadataUpdatedEvent<TExtensions>
where
    TExtensions: Send + Sync,
{
    async fn call(&self, events: Vec<EventResult>) -> EventCallbackResult<()> {
        ...
        // transfer events with 2 indexed topics cant decode
        // transfer events with 3 indexed topics
        let result: Vec<MetadataUpdatedResult> = events
            .into_iter()
            .filter_map(|item| {
                item.decoded_data
                    .downcast::<MetadataUpdatedData>()
                    .ok()
                    .map(|arc| MetadataUpdatedResult {
                        event_data: (*arc).clone(),
                        tx_information: item.tx_information,
                    })
            })
        ...
        if result.len() == events_len {
            ...
            panic!("MetadataUpdatedEvent: Unexpected data type - expected: MetadataUpdatedData")
        }
    }
}

pub enum ERC20EventType<TExtensions>
where
    TExtensions: 'static + Send + Sync,
{
    Transfer(TransferEvent<TExtensions>),
    MetadataUpdated(MetadataUpdatedEvent<TExtensions>),
}

impl<TExtensions> ERC20EventType<TExtensions>
where
    TExtensions: 'static + Send + Sync,
{
    pub fn topic_id(&self) -> &'static str {
        match self {
            ERC20EventType::Transfer(_) => {
                "0xddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a4df523b3ef"
            }
            ERC20EventType::MetadataUpdated(_) => {
    ...
    pub fn event_name(&self) -> &'static str {
        match self {
            ERC20EventType::Transfer(_) => "Transfer",
            ERC20EventType::MetadataUpdated(_) => "MetadataUpdated",
        }
    }
    ...
    fn decoder(
        ...
        match self {
            ERC20EventType::Transfer(_) => Arc::new(move |topics: Vec<B256>, data: Bytes| {
            ...
            ERC20EventType::MetadataUpdated(_) => {
                Arc::new(move |topics: Vec<B256>, data: Bytes| {
                    match MetadataUpdatedData::decode_raw_log(topics, &data[0..]) {
                        Ok(event) => {
                            let result: MetadataUpdatedData = event;
                            Arc::new(result) as Arc<dyn Any + Send + Sync>
                        }
                        Err(error) => Arc::new(error) as Arc<dyn Any + Send + Sync>,
                    }
    ...
    pub async fn register(self, manifest_path: &PathBuf, registry: &mut EventCallbackRegistry) {
        ...
        let (callback, reorg_sender): (
            ...
            ERC20EventType::MetadataUpdated(event) => {
                let reorg_sender = Some(event.context.reorg_tx.clone());
                let event = Arc::new(event);
                let callback = Arc::new(move |result| {
                    let event = Arc::clone(&event);
        ...
        registry.register_event(EventCallbackRegistryInformation {
            id: generate_random_id(10),
            indexer_name: "reliability".to_string(),
